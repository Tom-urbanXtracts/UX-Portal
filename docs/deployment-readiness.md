...
- Deploy `portal-product-content` with JWT gateway verification disabled and the server-only `MONDAY_PRODUCT_SECRET` for push compatibility. The primary direct path reads the pinned Monday product board through the dedicated OAuth app when an authorized Administrator selects **Sync catalog content**. Catalog managers may review one-to-one product-name and Brand suggestions, but the server applies zero automatic associations: an authorized reviewer must confirm a live pair before the exact Canix Item ID and Draft state are written to Monday. Both paths require factual content keyed by the current Canix item ID. Only explicitly published Monday records join the catalog; every accepted update is audited.
...
- Run the Catalog identity review. Confirm rows in No Longer Valid and Non-Cannabis are excluded, a product-name match with a different Brand is blocked, duplicate-name collisions are blocked, and no suggestion writes until an authorized catalog manager selects Verify & link. Confirm the approved pair is written to Monday as Draft and is attributed in the audit history.
