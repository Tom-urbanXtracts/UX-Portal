...
- `MONDAY_STATUS_SECRET`, `ORDER_SYNC_CRON_SECRET`, and `MONDAY_PRODUCT_SECRET` are configured server-side. The independent Vault-backed five-minute outbox job is active and returned HTTP 200 during its controlled empty-queue test. Signed Monday order callbacks are operational; product-content ingestion remains empty until its sending workflow and records are connected and tested.
