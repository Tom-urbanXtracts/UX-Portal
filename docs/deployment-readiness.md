...
- The wholesale pricing source audit is recorded in `docs/wholesale-pricing-source.md`. All 118 source rows are staged in the protected Pricing review queue. A row cannot publish until its current Canix Item ID is verified; published defaults remain subordinate to approved store-specific prices.
