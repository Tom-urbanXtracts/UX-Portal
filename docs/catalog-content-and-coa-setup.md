...
Internal users with `catalog.manage` also have a **Monday to Canix review queue** in Catalog. It compares current non-sample Canix items with Monday rows outside **No Longer Valid** and **Non-Cannabis**, then proposes only one-to-one product-name matches whose Brand also agrees. Punctuation-normalized matches are labelled separately. Brand conflicts and duplicate-name collisions remain blocked, and the audit reports `automaticMappingsApplied: 0`; a suggestion is never an authoritative association by itself. Selecting **Verify & link** rechecks the pair against both live sources, writes the confirmed Canix Item ID and `Draft` publication state to Monday, creates the protected product-content record, and records the reviewer in the administrative audit. Publication remains a separate deliberate action.
