...
Internal users with `catalog.manage` also have a **Monday to Canix review queue** in Catalog. It compares current non-sample Canix items with Monday rows outside **No Longer Valid** and **Non-Cannabis**. The queue returns every eligible Monday row without a Canix Item ID—not only actionable suggestions—and can be searched, filtered by review state, sorted by state, product, Brand, or Monday group, and exported as a spreadsheet-safe CSV containing candidate IDs plus blank reviewer-decision columns. The export follows the current search and review-state filters and neutralizes values that spreadsheet software could interpret as formulas. Each row explains why it is awaiting review and shows its closest exact or normalized Canix name candidate when one exists. Selecting **Approve exact matches** processes only unique exact product-name + Brand pairs; a Canix item with multiple Brand values, a Brand conflict, a reused Canix ID, or a duplicate name remains blocked from automatic approval. Punctuation-normalized matches are labelled separately and require **Verify & link**. An authorized catalog manager may resolve a blocked exception by entering the authoritative current Canix Item ID and a review note; the server rejects stale or reused IDs, rechecks both sources, keeps the record Draft, and records the decision separately as a manual audit event. Every approval path writes the confirmed Canix Item ID and `Draft` publication state to Monday, creates the protected product-content record, and records the approving user. Publication remains a separate deliberate action.

The board's existing file links use Monday's `protected_static` host and redirect an unauthenticated request to Monday sign-in. They are not valid retailer-facing catalog assets and must not be added to `PORTAL_EXTERNAL_ASSET_HOSTS`. **Portal Image URL** may point only to an IT-approved exact-host CDN or to a portal-controlled active asset. Portal-managed uploads are disabled until an approved content scanner is connected; when enabled, a different authorized reviewer must approve each file. Quarantined objects retain for 90 days and superseded archives for 365 days. Until the scanner is validated and product-image hold is lifted, images remain blank while Canix-supplied structured COA data and approved links continue to display.
