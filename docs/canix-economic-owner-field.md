...
- **Canix Owner** is the Canix user assigned to a package for operational organization and reporting. Canix Reporting exposes it as `owner_id` / `owner_name`; the public REST Package schema does not currently list an Owner property. It is not a company, title holder, risk bearer, or settlement party.
...
Canix documents Brand as an Item field and exposes `brand` on Item and Package API objects. It also has a separate Assign Package Owners feature, but the owner is a Canix user and is described as the person working on the package. The connected Reporting service exposes that operational assignment. The public REST Package schema version 1.3.11 does not list `owner`, `owner_id`, or `owner_name`, and neither source documents an organization-valued package or item field for economic ownership.

The connected Canix Reporting `inventory` schema was checked again on 2026-09-14. `package_inventory_facts_current` includes `owner_id` and `owner_name`, described by Canix as the assigned user's ID and display name. The latest comparison found 474 assigned rows out of 1,409 active, non-sample, non-volume packages, all assigned to the operational Canix user `urbanXtracts`. A forced REST-backed portal sync completed afterward with zero owner assignments across 1,023 usable packaged/each packages. UX OS already provides the **Canix Owner** filter, blank-assignment quick view, package column, CSV field, and package detail, but those remain blank until a supported Reporting bridge or documented REST property is connected. It never promotes the value to Economic Owner.
