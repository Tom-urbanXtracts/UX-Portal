...
- The portal sends Google's `prompt=select_account` and `hd=urbanxtracts.com` authorization hints so a personal Google session is not silently reused. The `hd` value improves account selection but is not an authorization control; the portal still verifies the returned email domain and server-owned workforce profile.
