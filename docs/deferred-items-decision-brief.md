...
| P0 before live pricing | Wholesale price-to-Canix crosswalk | All 118 `ACTIVE CART` rows are staged in a protected portal queue. Five unique exact product-name + Brand identities are verified and published; 113 exceptions remain unpublished. Store-specific approved prices take precedence | Sales Operations / Data | Review the remaining non-exact identities, then publish each verified default price without touching store-specific approvals |
| P0 before live QuickBooks views | QuickBooks portal OAuth and customer identity mapping | OAuth state, encrypted connection custody, API routing, and snapshots are now isolated by an explicit sandbox/production environment. The US static-egress proxy accepts both exact Intuit hosts and rejects unspecified or arbitrary destinations. The portal remains disconnected and no successful financial snapshot exists. Production credentials remain locked behind the company attestations and Intuit review | Finance / IT / Sales Operations | Authorize an Intuit sandbox company and complete the recorded connect/error/reconnect/access-scope checklist; then complete production review, change the environment and credentials together, authorize `URBANXTRACTS INC`, resolve duplicate licenses, and populate verified Customer IDs |
