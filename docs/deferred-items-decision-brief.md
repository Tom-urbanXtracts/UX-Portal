...
| --- | --- | --- | --- | --- |
| P0 before production deployment | Final hostname and Auth redirect list | `portal.urbanxtracts.com` is registered, its Wix DNS records resolve, and SSL is active; the hosting provider is completing its final redeploy. SSO/Auth cutover remains intentionally deferred | Administration / IT | Verify the custom host serves the current release, then add the final Auth redirect when SSO work resumes and choose when to retire the preview URL |
| P0 before production deployment | Public onboarding anti-abuse | The signed-out form renders configured Turnstile, passes an action-bound single-use token, verifies the exact hostname without visitor IP, and applies an atomic HMAC-scoped daily limit. Enforcement remains dormant until production credentials are installed | Administration / IT / Security | Create the production widget for `portal.urbanxtracts.com`, install its keys, use the default limit of 3 unless changed, run the end-to-end test, and enable both fail-closed flags |
