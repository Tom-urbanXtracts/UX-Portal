...
| P0 before live pricing | Wholesale price-to-Canix crosswalk | All 118 `ACTIVE CART` rows are staged in a protected portal queue with section-derived Brand, live Canix candidates, manual verification notes, and a separate publication gate. Store-specific approved prices take precedence. Unverified rows remain non-orderable | Sales Operations / Data | Review the remaining non-exact identities, then publish verified default prices without touching store-specific approvals |
