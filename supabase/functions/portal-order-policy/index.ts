...
function serialize(store: Row): Row {
  ...
  return {
    ...
    thresholdCents: threshold === null || threshold === undefined
      ? null
      : Number(threshold),
    mode: threshold === null || threshold === undefined
      ? "disabled"
      : Number(threshold) === 0
      ? "all_buyer_orders"
      : "above_threshold",
...
async function auditCasePolicy(
  caller: Caller,
  store: Row,
  enabled: boolean,
): Promise<void> {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (request.method === "GET") {
      return json(request, {
        policies: (await storesFor(caller)).map(serialize),
      });
    }
    ...
    if (caller.profile.role !== "internal" || !caller.canManage) {
      return json(request, {
        error:
          "Only Administrator, Operations, or Sales may change store approval policy.",
      }, 403);
    }
    const body = await request.json() as Row;
    const action = String(body.action || "");
    const locationLicense = String(body.locationLicense || "").trim().slice(
      0,
      120,
    );
    ...
    if (action === "update-case-policy") {
      if (typeof body.enforceCaseQuantity !== "boolean") {
        return json(request, {
          error: "Case enforcement must be true or false.",
        }, 400);
      }
    ...
    if (action !== "update") {
      return json(request, { error: "Unsupported policy action" }, 400);
    }
    ...
    if (mode === "disabled") thresholdCents = null;
      ...
      if (
        ...
        return json(request, {
          error: "Enter a valid positive approval threshold.",
        }, 400);
      }
    } else {return json(request, {
        error: "Choose a supported approval policy.",
      }, 400);}
