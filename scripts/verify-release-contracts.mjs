...
assertContract(productContent.includes("catalogMappingAudit") && productContent.includes('matchKind: "exact_name_brand"') && productContent.includes("automaticMappingsApplied: 0") && productContent.includes("mondayMappingEligible"), "catalog identity suggestions never become automatic links and exclude invalid Monday groups");
...
assertContract(source.includes("Monday to Canix review queue") && source.includes("Verify &amp; link") && source.includes("catalogMappingPending"), "catalog managers have a protected human-review identity queue");
